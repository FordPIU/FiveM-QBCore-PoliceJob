...
-- Functions
function IsLawEnforcement(playerData, wantOnDuty)
    playerData = playerData or QBCore.Functions.GetPlayerData()
    wantOnDuty = wantOnDuty or false

    if playerData.job.name == "lspd" or playerData.job.name == "bcso" then
        if wantOnDuty then
...
function isOnDuty()
    ...
    if pData ~= nil then
        if pData.job ~= nil then
            return pData.job.onduty
        else
            return false
        end
    else
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob and not IsLawEnforcement() then
        if DutyBlips then
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    if (JobInfo.name == "lspd" and PlayerJob.name ~= "lspd") or (JobInfo.name == "bcso" and PlayerJob.name ~= "bcso") then
        if JobInfo.onduty then
            TriggerServerEvent("QBCore:ToggleDuty")
        end
    end

    if JobInfo.name ~= "lspd" or JobInfo.name ~= "bcso" then
        if DutyBlips then
            for _, v in pairs(DutyBlips) do
                RemoveBlip(v)
            end
...
RegisterNetEvent('police:client:UpdateBlips', function(players)
    if PlayerJob and (PlayerJob.name == 'lspd' or PlayerJob.name == 'bcso' or PlayerJob.name == 'ambulance') and
        isOnDuty() then
        if DutyBlips then
            for _, v in pairs(DutyBlips) do
                RemoveBlip(v)
...
CreateThread(function()
    for _, dept in pairs(Config.Departments) do
        for _, station in pairs(dept.Stations) do
            ...
            SetBlipAsShortRange(blip, true)
            SetBlipScale(blip, 0.8)
            SetBlipColour(blip, station.blipColor)
            BeginTextCommandSetBlipName("STRING")
