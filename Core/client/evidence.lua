...
local function DropBulletCasing(weapon, ped)
    local randX = GetRandomFloatInRange(-0.5, 0.5)
    local randY = GetRandomFloatInRange(-0.5, 0.5)
...
-- Events
RegisterNetEvent("evidence:client:CasingCreatedSetGrav", function(NetId)
...
RegisterNetEvent('evidence:client:RemoveCasing', function(casingId, netObj)
    Casings[casingId] = nil
    DeleteObject(NetworkGetEntityFromNetworkId(netObj))
    CurrentCasing = 0
end)
...
CreateThread(function() -- Gunpowder Status when shooting
    while true do
        ...
        for _,ped in pairs(GetGamePool("CPed")) do
            if IsPedShooting(ped) and (NetworkGetEntityOwner(ped) == PlayerId() or ped == PlayerPedId()) then
                ...
                if not WhitelistedWeapon(weapon) then
                    ...
                    if shotAmount > 5 and (CurrentStatusList == nil or CurrentStatusList['gunpowder'] == nil) then
                        if math.random(1, 10) <= 7 and ped == PlayerPedId() then
...
CreateThread(function()
    while true do
        ...
        if CurrentCasing ~= nil then
            DrawText3D(CurrentCasing.position[1], CurrentCasing.position[2], CurrentCasing.position[3], Lang:t('info.bullet_casing', {value = CurrentCasing.data.type}))
            if IsControlJustReleased(0, 47) then
                local s1, s2 = GetStreetNameAtCoord(CurrentCasing.position[1], CurrentCasing.position[2], CurrentCasing.position[3])
                ...
                local info = {
                    ...
                    ammolabel = Config.AmmoLabels[QBCore.Shared.Weapons[CurrentCasing.data.type]['ammotype']],
                    ammotype = CurrentCasing.data.type,
                    serie = CurrentCasing.data.serie
                }
                TriggerServerEvent('evidence:server:AddCasingToInventory', CurrentCasing, info, NetworkGetNetworkIdFromEntity(CurrentCasing.entity))
            end
        end
...
CreateThread(function()
    while true do
        Wait(10)
        if LocalPlayer.state.isLoggedIn then
            if IsLawEnforcement(nil, true) then
                CurrentCasing = nil
                if IsPlayerFreeAiming(PlayerId()) and GetSelectedPedWeapon(PlayerPedId()) == `WEAPON_FLASHLIGHT` then
                    for _,v in pairs(GetGamePool("CObject")) do
                        if Entity(v).state.casingdata ~= nil then
                            local pos = GetEntityCoords(PlayerPedId(), true)
                            local pos2 = GetEntityCoords(v)

                            local dist = #(pos - pos2)
                            if dist < 2.0 then
                                CurrentCasing = {
                                    data = Entity(v).state.casingdata,
                                    position = pos2,
                                    entity = v,
                                }
                            end
