...
local function UpdateBlips()
    ...
    for _, v in pairs(players) do
        if v and ((v.PlayerData.job.name == "lspd" or v.PlayerData.job.name == "bcso") or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
...
local function GetCurrentCops()
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "lspd" or v.PlayerData.job.name == "bcso") and v.PlayerData.job.onduty then
            amount += 1
        end
    end
    return amount
...
QBCore.Commands.Add("spikestrip", Lang:t("commands.place_spike"), {}, false, function(source)
    ...
    if (Player.PlayerData.job.name == "lspd" or Player.PlayerData.job.name == "bcso") and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if (Player.PlayerData.job.name == "lspd" or Player.PlayerData.job.name == "bcso") and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if (Player.PlayerData.job.name == "lspd" or Player.PlayerData.job.name == "bcso") or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if not OtherPlayer or (Player.PlayerData.job.name ~= "lspd" and Player.PlayerData.job.name ~= "bcso") or not Player.PlayerData.job.onduty then return end
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.Functions.GetItemByName("moneybag") or not item.info or item.info == "" or (Player.PlayerData.job.name == "lspd" or Player.PlayerData.job.name == "bcso") or not Player.Functions.RemoveItem("moneybag", 1, item.slot) then return end
...
QBCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(_, cb)
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "lspd" or v.PlayerData.job.name == "bcso") and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName("handcuffs") and (Player.PlayerData.job.name ~= "lspd" and Player.PlayerData.job.name ~= "bcso")) then return end
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    if ((Player.PlayerData.job.name == "lspd" or Player.PlayerData.job.name == "bcso") or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or (Player.PlayerData.job.name ~= "lspd" and Player.PlayerData.job.name ~= "bcso") then return end
...
RegisterNetEvent('evidence:server:CreateCasing', function(weapon, coords)
    ...
    local object = CreateObject(`prop_sgun_casing`, coords.x, coords.y, coords.z - 1, true, true, true)
    repeat
        Wait(0)
    until DoesEntityExist(object)
    Entity(object).state.casingdata = {
        type = weapon,
        serie = serieNumber and serieNumber or "No Serial Number"
    }
    TriggerClientEvent("evidence:client:CasingCreatedSetGrav", src, NetworkGetNetworkIdFromEntity(object))
    --TriggerClientEvent("evidence:client:AddCasing", source, casingId, weapon, coords, serieNumber)
end)
...
RegisterNetEvent('evidence:server:AddCasingToInventory', function(casingId, casingInfo, netObj)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        if Player.Functions.AddItem("filled_evidence_bag", 1, false, casingInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items["filled_evidence_bag"], "add")
            TriggerClientEvent("evidence:client:RemoveCasing", -1, casingId, netObj)
...
RegisterNetEvent('cpolice:server:CanTakeOutVehicle', function(vehicleHash, DeptConfig, eparam1)
    local DeptName = string.lower(DeptConfig.PlainText)
    local DeptFunds = exports["qb-management"]:GetAccount(DeptName)
    local VehiclePrice = math.floor(DeptConfig.VehicleSettings[vehicleHash].Price * (DeptConfig.VehicleEquipmentMultiplier + 1.0))

    if DeptFunds >= VehiclePrice then
        exports["qb-management"]:RemoveMoney(DeptName, VehiclePrice)

        print("Dept " .. DeptName .. " checked out a vehicle for $" .. VehiclePrice)

        TriggerClientEvent("cpolice:client:TakeOutVehicle", source, vehicleHash, eparam1)
    else
        print("Dept " .. DeptName .. " has insufficent funds for a vehicle that costs $" .. VehiclePrice)

        TriggerClientEvent("cpolice:client:RejectTakeOutVehicle", source)
    end
end)

RegisterNetEvent('cpolice:server:ReturnVehicle', function(vehicleH, DeptConfig)
    local DeptName = DeptConfig.PlainText:lower()
    local vehicleHash = DeptConfig.VehicleHashes[vehicleH]

    if DeptConfig.VehicleSettings[vehicleHash] ~= nil then
        local VehiclePrice = math.floor(DeptConfig.VehicleSettings[vehicleHash].Price * (DeptConfig.VehicleEquipmentMultiplier + 1.0))

        exports["qb-management"]:AddMoney(DeptName, VehiclePrice)

        print("Dept " .. DeptName .. " returned a vehicle for $" .. VehiclePrice)
    end
end)